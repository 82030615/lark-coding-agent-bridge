...
program
  ...
  .option('--agent <kind>', 'agent kind for a new profile (claude, codex, or codebuddy)')
...
program
  ...
  .option('--agent <kind>', 'agent kind for legacy v1 profile migration (claude, codex, or codebuddy)')
...
profile
  ...
  .option('--agent <kind>', 'agent kind (claude, codex, or codebuddy)')
...
program
  ...
  .option('--agent <kind>', 'agent kind for first-run profile bootstrap (claude, codex, or codebuddy)')
  .option('--workspace <path>', 'initial working directory for first-run profile bootstrap')
  .option('--app-id <id>', 'use an existing Lark/Feishu app instead of QR app creation')
  .option('--app-secret <secret>', 'App Secret for --app-id; prefer interactive input on shared machines')
  .option('--tenant <tenant>', 'tenant for --app-id (feishu or lark; default feishu)')

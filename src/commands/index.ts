...
import { claudeCapability, codebuddyCapability, codexCapability, type AgentCapabilityId } from '../agent/capability';
...
async function applyResume(sessionId: string, ctx: CommandContext): Promise<void> {
  if (ctx.sessionCatalog && ctx.sessionCatalogIdentity) {
    ...
    ctx.activeRuns.interrupt(ctx.scope);
    if (ctx.sessionCatalogIdentity.agentId === 'claude' || ctx.sessionCatalogIdentity.agentId === 'codebuddy') {
...
async function handleDoctor(args: string, ctx: CommandContext): Promise<void> {
  ...
  const capability =
    ctx.controls.profileConfig.agentKind === 'codex'
      ...
      : ctx.controls.profileConfig.agentKind === 'codebuddy'
        ? codebuddyCapability(ctx.controls.profileConfig)
